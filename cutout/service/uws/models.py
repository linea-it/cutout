...
def _convert_job(job: SQLJob) -> Job:
    ...
    """
    # TODO: Criar a classe de erro e gerenciar os errors do Job
    error = None
    ...
    return Job(
        ...
        execution_duration=job.execution_duration.total_seconds() if job.execution_duration != None else 0,
        quote=job.quote,
        parameters=[
            JobParameter(parameter_id=p.parameter, value=p.value, is_post=p.is_post)
