...
def get_fits_data(RA, DEC, size, tiles, band, path):
    ...
    if len(tile_un) == 1:
        ...
        data_1, wcs_1 = cutout_fits(RA, DEC, size,
                                    tile_un[0], band, path, 'trim')
        data_2, wcs_2 = cutout_fits(RA, DEC, size,
                                    tile_un[1], band, path, 'trim')
        ...
        data_2, wcs_2 = cutout_fits(RA, DEC, size,
                                    tile_un[1], band, path, 'trim')
        data_3, wcs_3 = cutout_fits(RA, DEC, size,
                                    tile_un[2], band, path, 'trim')

        # Biggest at bottom:
        if np.shape(data_1)[1] < np.shape(data_3)[1]:
            data_12 = np.concatenate((data_1[118:, :-118],
                                      data_2[118:, 118:]),
                                     axis=1)
            data_ = np.concatenate((data_3[:-118, :],
                                    data_12[:, 0:np.shape(data_3)[1]]),
                                   axis=0)
        # Biggest at top:
        else:
            data_23 = np.concatenate((data_2[:-118, 118:],
                                      data_3[:-118, :-118]), axis=1)
            data_ = np.concatenate((data_23,
                                    data_1[118:, 0:np.shape(data_23)[1]]),
                                   axis=0)
        return(data_, wcs_3)
...
def cutout_lupton(g_data, r_data, i_data, minimum, stretch, Q, filename):
    ...
    rgb_default = make_lupton_rgb(
        i_data, r_data, g_data, minimum=minimum, stretch=stretch, Q=Q, filename=filename)
...
def cutout_fits(RA_center, DEC_center, size_arcmin, tile_name, band, path, mode='partial'):
    ...
    cutout1 = Cutout2D(fits.getdata(file_name, ext=0),
                       (SkyCoord(ra=RA_center*u.degree,
                        dec=DEC_center*u.degree, frame='icrs')),
                       size_arcmin*u.arcmin, wcs=wcs,
                       mode=mode)
...
def cutout_verts(RA_center, DEC_center, size_arcmin):
    ...
    for i, j in enumerate(RA_center):
        ...
        sep = 0.5 * np.sqrt(2.) * size_arcmin[i] * u.arcmin
...
def tiles_from_cat(cat, file_path):
    ...
    tile_names = np.loadtxt(file_path, usecols=(
        2), delimiter=',', dtype=str, unpack=True)
    ...
    for i in range(np.shape(ra)[0]):
        idx_ = []
        for j in range(4):
            idx_.append(np.argwhere((ra_ll < ra[i][j]) & (ra_ur > ra[i][j]) & (
                dec_ll < dec[i][j]) & (dec_ur > dec[i][j]))[0][0])
